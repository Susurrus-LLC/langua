  ...
  return (
    <Layout>
      ...
      <Help toolInfo={metadata.toolInfo.gen}>
        ...
        <p>
          The syntax is identical for all Pattern and Subpattern fields.
          Uppercase versions of the standard English letters (e.g.{' '}
          <code>C</code>, <code>V</code>, or <code>N</code>) are variables
          referring to Subpatterns while any other letter (e.g. <code>a</code>,{' '}
          <code>s</code>, or <code>n</code>) represents that specific glyph.
        </p>
        <h5 id='options'>
        ...
        <p>
          ...
          default, each option will have the same chance of being chosen. This
          can be changed by assigning <a href='#weights'>weights</a>. For
          example, with the Subpatterns <code>V: a/i</code>, <code>C: t/s</code>
          , and <code>N: m/n</code> and the Pattern <code>CVN</code>, the tool
          will output the words <samp>sam tan tam tin sim san tim sin</samp>.
        </p>
        <h5 id='units'>
        ...
        <p>
          Everything contained within brackets (<code>[ ]</code>) is treated as
          a single unit. For example, the Pattern <code>as[tu/top/kan]</code>{' '}
          will produce the output <code>astu astop askan</code>. Brackets and
        ...
        <Notice>This functionality has not yet been implemented.</Notice>
        <p>
          Everything contained within parentheses (<code>( )</code>) is treated
          as a single unit that is optional. For example, the Pattern{' '}
          <code>as(tu/top/kan)</code> will produce the output{' '}
          <code>as astu astop askan</code>. Parentheses and brackets can be
          nested unlimitedly.
        </p>
        <h5 id='weights'>
        ...
        <p>
          ...
          <code>*</code> and a number to improve the likelihood of it being
          chosen. For example, with the pattern <code>a/e*3/i*2/o/u*5</code>,
          the tool would output <code>a</code> or <code>o</code> with equal
          probability, but compared to these would be twice as likely to output{' '}
          <code>i</code>, three times as likely to output <code>e</code>, and
          five times as likely to output <code>u</code>. The number used as a
          weight must be an integer between 1 and 128.
        </p>
        <h5 id='filtering'>
        ...
        <p>
          ...
          potential output to restrict the possible results. Multiple filters
          can be added to the same unit to filter out multiple options. For
          example, with the Subpattern <code>V: a/i/u</code> and hte Pattern{' '}
        ...
        <p>
          Once the Pattern and all Subpatterns have been set up, you can click
          the <strong>Generate</strong> button to generate words. There are also
        ...
        <p>
          ...
          including how many words were printed to the output, how many
          duplicates were filtered out of the results (if any), and how many
          words are possible based on the given Pattern and Subpatterns. (Note
          that the number of <em>unique</em> words possible may actually be
          lower if there are multiple ways to obtain the same word.)
        </p>
        <h4 id='files'>Saving and Loading Settings</h4>
        <p>
          Clicking the <strong>Save</strong> button will save the current
          settings to the browser’s local storage and generate a small{' '}
          <code>.lngg</code> text file containing the current settings that can
          be saved to your disk. This <code>.lngg</code> file can be loaded
          using the <strong>Open</strong> button to reload saved settings.
        </p>
