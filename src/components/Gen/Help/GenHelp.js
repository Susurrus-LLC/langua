...
const GenHelp = (props) => (
  <div className={props.classes.genHelp}>
    ...
    <p>Multiple options can be separated using forward slashes (<code>/</code>). For each word, the tool will randomly select one of the options. By default, each option will have the same chance of being chosen. This can be changed by assigning <a href='#weights'>weights</a>. For example, with the Subpatterns <code>V: a/i</code>, <code>C: t/s</code>, and <code>N: m/n</code> and the Pattern <code>CVN</code>, the tool will output the words <samp>sam tan tam tin sim san tim sin</samp>.</p>
    <h5 id='units'>Single Units &ndash; <code>[ ]</code></h5>
    <Notice>This functionality has not yet been implemented.</Notice>
    <p>Everything contained within brackets (<code>[ ]</code>) is treated as a single unit. For example, the Pattern <code>as[tu/top/kan]</code> will produce the output <code>astu astop askan</code>. Brackets and parentheses can be nested unlimitedly.</p>
    <h5 id='optional'>Optional Units &ndash; <code>( )</code></h5>
    <Notice>This functionality has not yet been implemented.</Notice>
    <p>Everything contained within parentheses (<code>( )</code>) is treated as a single unit that is optional. For example, the Pattern <code>as(tu/top/kan)</code> will produce the output <code>as astu astop askan</code>. Parentheses and brackets can be nested unlimitedly.</p>
    <h5 id='weights'>Weights &ndash; <code>*</code></h5>
    <Notice>This functionality has not yet been implemented.</Notice>
    <p>Weights can be added to certain choices using an asterisk <code>*</code> and a number to improve the likelihood of it being chosen. For example, with the pattern <code>a/e*3/i*2/o/u*5</code>, the tool would output <code>a</code> or <code>o</code> with equal probability, but compared to these would be twice as likely to output <code>i</code>, three times as likely to output <code>e</code>, and five times as likely to output <code>u</code>. The number used as a weight must be an integer between 1 and 128.</p>
    <h5 id='filtering'>Filtering &ndash; <code>^</code></h5>
    <Notice>This functionality has not yet been implemented.</Notice>
    <p>Filtering can be added to units using a caret <code>^</code> and a potential output to restrict the possible results. Multiple filters can be added to the same unit to filter out multiple options. For example, with the Subpattern <code>V: a/i/u</code> and hte Pattern <code>t[VV]^aa^ii^uu</code>, the tool will output <code>tai tau tia tiu tua tui</code>, filtering out <code>taa tii tuu</code>.</p>
    <h5 id='escaping'>Esaping Characters &ndash; <code>&quot; &quot;</code></h5>
    <Notice>This functionality has not yet been implemented.</Notice>
    <p>Special characters otherwise used for the tool&rsquo;s syntax can be escaped using double quotes <code>&quot; &quot;</code>. For example, with the Subpattern <code>V: a/i/u</code> and the Pattern <code>Vt&quot;V[n/m]&quot;</code>, the tool will produce the output <code>atV[n/m] itV[n/m] utV[n/m]</code>.</p>
    ...
    <p>Clicking the <strong>Save</strong> button will save the current settings to the browser’s local storage and generate a small <code>.lngg</code> text file containing the current settings that can be saved to your disk. This <code>.lngg</code> file can be loaded using the <strong>Open</strong> button to reload saved settings.</p>
  </div>
)
